...
class TestCaseDocumentSubmission(CustomPKModel, AuditMixin):
    """Test case document submissions with version tracking"""
    __tablename__ = 'cycle_report_test_cases_document_submissions'
    ...
    # Version chain
    replaces_submission_id = Column(UUID, ForeignKey('cycle_report_test_cases_document_submissions.submission_id'))
...
class TestCaseDocumentRevisionHistory(CustomPKModel):
    """Track all test case document changes"""
    __tablename__ = 'cycle_report_test_case_document_revision_history'
    
    history_id = Column(UUID, primary_key=True, default=uuid.uuid4)
    submission_id = Column(UUID, ForeignKey('cycle_report_test_cases_document_submissions.submission_id'), nullable=False)
...
class TestExecutionAudit(CustomPKModel, AuditMixin):
    ...
    cycle_id = Column(Integer, ForeignKey('test_cycles.cycle_id'), nullable=False)
    report_id = Column(Integer, ForeignKey('reports.id'), nullable=False)
    test_execution_id = Column(Integer, ForeignKey('cycle_report_test_executions.execution_id'))
...
class ObservationDecision(CustomPKModel, AuditMixin):
    ...
    observation_id = Column(Integer, ForeignKey('cycle_report_observations.observation_id'))
