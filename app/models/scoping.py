"""
Scoping phase models for workflow management - Consolidated Version Management System

This module contains the consolidated scoping models that implement the versioning framework pattern
used across the application, providing comprehensive scoping decision management.
"""
...
class VersionStatus(str, Enum):
    DRAFT = "draft"
    PENDING_APPROVAL = "pending_approval"  # Match SQLAlchemy enum definition
    APPROVED = "approved"
    REJECTED = "rejected"
...
# PostgreSQL enum types (for backward compatibility)
scoping_recommendation_enum = PgEnum('test', 'skip', name='scoping_recommendation_enum')
scoping_decision_enum = PgEnum('accept', 'decline', 'override', name='scoping_decision_enum')
approval_status_enum = PgEnum('approved', 'rejected', 'pending', 'needs_revision', name='approval_status_enum')
...
class AttributeScopingRecommendation(ScopingAttribute):
    """Legacy alias for backward compatibility"""
    pass

class ScopingDecision(ScopingAttribute):
    """Legacy alias for backward compatibility"""
    pass

class TesterScopingDecision(ScopingAttribute):
    """Legacy alias for backward compatibility"""
    pass

class ScopingSubmission(ScopingVersion):
    """Legacy alias for backward compatibility"""
    pass

class ReportOwnerScopingReview(ScopingVersion):
    """Legacy alias for backward compatibility"""
    pass


class ScopingAuditLog(CustomPKModel, AuditMixin):
    """Audit log for scoping phase actions"""
    
    __tablename__ = "scoping_audit_log"
    ...
    def __repr__(self):
        return f"<ScopingAuditLog(id={self.audit_id}, action='{self.action}')>" 