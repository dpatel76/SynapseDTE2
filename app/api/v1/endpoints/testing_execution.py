"""
Testing Execution Engine API endpoints
"""
...
from app.models.testing import CycleReportSampleSelectionSamples
from app.models.sample_selection import SampleRecord
from app.models.testing_execution import (
    TestingExecutionPhase, TestExecution as TestingTestExecution, DocumentAnalysis,
    DatabaseTest, TestResultReview, TestComparison, BulkTestExecution, TestingExecutionAuditLog
)
from app.schemas.testing_execution import (
    TestingExecutionPhaseStart, TestingExecutionPhaseStatus, TestingExecutionPhaseComplete,
    ...
    TestingAnalytics, TestingExecutionAuditLog as TestingExecutionAuditLogSchema,
...
async def log_audit_action(
    ...
    audit_log = TestingExecutionAuditLog(
        cycle_id=cycle_id,
        report_id=report_id,
        phase_id=phase_id,
        action=action,
...
# Testing Execution Phase Management

@router.post("/{cycle_id}/reports/{report_id}/start", response_model=Dict[str, Any])
@require_permission("testing", "execute")
async def start_testing_execution_phase(
    cycle_id: int,
    report_id: int,
    phase_data: TestingExecutionPhaseStart,
    request: Request,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """Start Testing Execution phase"""
    start_time = time.time()
    
    try:
        # Check if phase already exists
        existing_phase = await db.execute(
            select(TestingExecutionPhase).where(
                and_(TestingExecutionPhase.cycle_id == cycle_id, TestingExecutionPhase.report_id == report_id)
            )
        )
        ...
        if existing_phase:
            raise HTTPException(
                ...
                detail="Testing Execution phase already exists for this cycle and report"
            )

        # Create new testing execution phase
        phase_id = str(uuid.uuid4())
        new_phase = TestingExecutionPhase(
            phase_id=phase_id,
            cycle_id=cycle_id,
            report_id=report_id,
        ...
        await log_audit_action(
            db, cycle_id, report_id, "START_TESTING_PHASE", "TestingExecutionPhase",
            ...
            "Testing Execution phase started", execution_time, phase_id
        )

        return {
            "message": "Testing Execution phase started successfully",
            "phase_id": phase_id,
            "status": "In Progress",
...
@router.get("/{cycle_id}/reports/{report_id}/status", response_model=TestingExecutionPhaseStatus)
@require_permission("testing", "read")
async def get_testing_execution_status(
    cycle_id: int,
    report_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """Get Testing Execution phase status and progress"""
    ...
    try:
        ...
        if not phase:
            raise HTTPException(
                ...
                detail="Testing Execution phase not found"
            )
        ...
        return TestingExecutionPhaseStatus(
            cycle_id=cycle_id,
            report_id=report_id,
...
async def get_submitted_test_cases(
    ...
    try:
        ...
        query = select(TestCase).options(
            selectinload(TestCase.data_provider),
        ...
        for tc in test_cases:
            ...
            case_data = {
                ...
                "data_provider": {
                    "user_id": tc.data_provider_id,
                    "name": f"{tc.data_provider.first_name} {tc.data_provider.last_name}" if tc.data_provider else "Unknown",
                    "email": tc.data_provider.email if tc.data_provider else None,
                    "first_name": tc.data_provider.first_name if tc.data_provider else None,
                    "last_name": tc.data_provider.last_name if tc.data_provider else None,
                },
                "attribute": {
    ...
    try:
        ...
        test_stats = await db.execute(
            select(
                ...
                func.count().filter(TestingTestExecution.test_type == 'Database Based').label("cycle_report_test_execution_database_tests"),
                func.count().filter(TestingTestExecution.status == 'Completed').label('completed_tests'),
                func.count().filter(TestingTestExecution.status == 'Failed').label('failed_tests'),
                func.avg(TestingTestExecution.confidence_score).label('avg_confidence'),
...
async def complete_testing_execution_phase(
    cycle_id: int,
    report_id: int,
    completion_data: TestingExecutionPhaseComplete,
    request: Request,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """Complete Testing Execution phase"""
    start_time = time.time()
    ...
    try:
        ...
        if phase.phase_status == "Completed":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Testing Execution phase is already completed"
            )
        ...
        execution_time = int((time.time() - start_time) * 1000)
        await log_audit_action(
            db, cycle_id, report_id, "COMPLETE_TESTING_PHASE", "TestingExecutionPhase",
        ...
        return {
            "message": "Testing Execution phase completed successfully",
            "phase_id": phase.phase_id,
...
@router.get("/{cycle_id}/reports/{report_id}/audit_logs", response_model=List[TestingExecutionAuditLogSchema])
@require_permission("testing", "read")
async def get_testing_execution_audit_logs(
    cycle_id: int,
    report_id: int,
    limit: int = 50,
    offset: int = 0,
    ...
    try:
        logs_result = await db.execute(
            select(TestingExecutionAuditLog)
            .where(and_(TestingExecutionAuditLog.cycle_id == cycle_id, TestingExecutionAuditLog.report_id == report_id))
            .order_by(TestingExecutionAuditLog.performed_at.desc())
            .limit(limit)
            .offset(offset)
            .options(selectinload(TestingExecutionAuditLog.user))
        )
        logs = logs_result.scalars().all()

        return [
            TestingExecutionAuditLogSchema(
                log_id=log.log_id,
...
async def get_test_case_revision_history(
    ...
    for idx, doc in enumerate(documents):
        ...
        revisions.append({
            ...
            "data_provider_id": doc.data_provider_id,
...
async def perform_real_document_analysis(
    ...
    if llm_available:
        ...
        report_result = await db.execute(
            select(Report).where(Report.report_id == test_case.report_id)
        )
        ...
        if report and report.regulation:
            # Extract regulatory report type and schedule from regulation field
            regulation_lower = report.regulation.lower()
            if 'fr y-14m' in regulation_lower or 'fr_y_14m' in regulation_lower:
                regulatory_report = 'fr_y_14m'
                # Try to extract schedule
                if 'schedule d.1' in regulation_lower or 'schedule_d_1' in regulation_lower:
                    regulatory_schedule = 'schedule_d_1'
                elif 'schedule a.1' in regulation_lower or 'schedule_a_1' in regulation_lower:
                    regulatory_schedule = 'schedule_a_1'
                elif 'schedule c.1' in regulation_lower or 'schedule_c_1' in regulation_lower:
                    regulatory_schedule = 'schedule_c_1'
        ...
        try:
            ...
            extraction_result = await llm_service.extract_test_value_from_document(
                ...
                regulatory_schedule=regulatory_schedule
            )
