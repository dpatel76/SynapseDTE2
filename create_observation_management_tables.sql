...
CREATE INDEX idx_observations_group_id ON cycle_report_observations(group_id);
CREATE INDEX idx_observations_test_execution_id ON cycle_report_observations(test_execution_id);
CREATE INDEX idx_observations_test_case_id ON cycle_report_observations(test_case_id);
CREATE INDEX idx_observations_attribute_id ON cycle_report_observations(attribute_id);
CREATE INDEX idx_observations_lob_id ON cycle_report_observations(lob_id);
CREATE INDEX idx_observations_sample_id ON cycle_report_observations(sample_id);

-- Create function to update observation counts
CREATE OR REPLACE FUNCTION update_observation_group_count()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trg_update_observation_count
    AFTER INSERT OR DELETE ON cycle_report_observations
    FOR EACH ROW EXECUTE FUNCTION update_observation_group_count();
...
COMMENT ON TABLE cycle_report_observations IS 'Individual observations, one per test case failure, linked to test execution results';
...
COMMENT ON COLUMN cycle_report_observations.test_execution_id IS 'Links to the test execution that generated this observation';
COMMENT ON COLUMN cycle_report_observations.confidence_level IS 'Confidence level (0.0-1.0) that this is a real issue';
COMMENT ON COLUMN cycle_report_observations.evidence_files IS 'JSON array of evidence file paths and metadata';
COMMENT ON COLUMN cycle_report_observations.variance_details IS 'JSON object with detailed variance information';