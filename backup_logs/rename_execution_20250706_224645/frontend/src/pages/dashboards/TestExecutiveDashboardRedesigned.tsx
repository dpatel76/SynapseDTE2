...
const TestExecutiveDashboardRedesigned: React.FC = () => {
  ...
  const getKeyMetrics = () => {
    ...
    if (selectedCycleId === 'all') {
      ...
      const activeCycles = cyclesData?.filter((c: any) => c.status === 'Active') || [];
      const completedCycles = cyclesData?.filter((c: any) => c.status === 'Completed') || [];
      const offTrackCycles = activeCycles.filter((c: any) => c.is_offtrack || c.at_risk_count > 0).length;
      
      return [
        {
          title: 'Total Cycles',
          value: cyclesData?.length || 0,
          icon: <Assignment />,
        ...
        },
        {
          ...
          subtitle: offTrackCycles > 0 ? `${offTrackCycles} off track` : undefined,
          subtitleColor: 'error'
        },
        {
        ...
        {
          title: 'Total Reports',
          value: metrics.total_reports || metrics.cycle_summary?.total_reports || 0,
          icon: <Assessment />,
          color: theme.palette.primary.main,
        },
        {
          ...
          subtitle: metrics.at_risk_reports > 0 ? `${metrics.at_risk_reports} at risk` : undefined,
          subtitleColor: 'error'
        },
        {
          title: 'Completed Reports',
          value: metrics.completed_reports || metrics.cycle_summary?.completed_reports || 0,
      ...
      return [
        {
          title: 'Total Reports',
          value: metrics.cycle_summary?.total_reports || 0,
          icon: <Assignment />,
          color: theme.palette.primary.main,
        },
        {
          title: 'Completion Rate',
          value: `${Math.round(metrics.cycle_summary?.completion_rate || 0)}%`,
          icon: <CheckCircle />,
          color: theme.palette.success.main,
        },
        {
          title: 'At Risk',
          value: metrics.sla_summary?.at_risk_count || 0,
          icon: <Warning />,
          color: theme.palette.warning.main,
          subtitle: 'reports'
        },
        {
          ...
          value: `${Math.round(metrics.sla_summary?.compliance_rate || 0)}%`,
          icon: <Speed />,
          color: theme.palette.info.main,
        },
        {
          title: 'Observations',
          value: metrics.quality_metrics?.total_observations || 0,
          icon: <BugReport />,
          color: theme.palette.error.main,
          subtitle: `${metrics.quality_metrics?.critical_issues || 0} critical`
        }
      ];
    }
  };
              ...
              >
                <MenuItem value="all">
                  <Box display="flex" alignItems="center" gap={1}>
                    <DashboardIcon />
                    <span>All Cycles (Aggregated)</span>
                  </Box>
                </MenuItem>
          ...
          <Paper sx={{ p: 3, mb: 4 }}>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
              ...
              <Box display="flex" gap={2} alignItems="center">
                <Chip
                  icon={<Warning />}
                  label={`${metrics?.at_risk_reports || 0} total at risk`}
                  color="error"
                  variant="outlined"
                />
