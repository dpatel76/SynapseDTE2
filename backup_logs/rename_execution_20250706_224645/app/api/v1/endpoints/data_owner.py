"""
Data Owner Identification phase endpoints
"""
...
from app.models.data_owner import (
    AttributeLOBAssignment, CDONotification, HistoricalDataOwnerAssignment,
...
from app.schemas.data_owner import (
    DataProviderPhaseStart, AttributeLOBAssignment as AttributeLOBAssignmentSchema,
    LOBAssignmentSubmission, CDONotificationData, DataOwnerAssignmentRequest,
    ...
    DataOwnerAssignmentAuditLog, AssignmentStatus, EscalationLevel
)
...
def require_tester_or_cdo(current_user: User = Depends(get_current_user)):
    """Require tester or Data Executive role"""
    if not (current_user.is_tester or current_user.is_cdo):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Tester or Data Executive role required"
        )
    return True
...
async def create_audit_log(
    ...
    """Create audit log entry for data owner phase actions"""
...
async def start_data_owner_phase(
    cycle_id: int,
    report_id: int,
    ...
    try:
        ...
        if cycle_report.tester_id != current_user.user_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only assigned tester can start data owner phase"
            )
        ...
        if not sampling_phase or sampling_phase.status != 'Complete':
            raise HTTPException(
                ...
                detail="Sampling phase must be completed before starting data owner identification"
            )

        # Check if data owner phase already exists and has been started
        ...
        if data_owner_phase and data_owner_phase.actual_start_date:
            raise HTTPException(
                ...
                detail="Data owner identification phase already started"
            )

        # If phase record exists but hasn't been started, update it instead of creating new one
        if data_owner_phase:
            # Update existing phase record
            data_owner_phase.status = 'In Progress'
            data_owner_phase.state = 'In Progress'
            data_owner_phase.actual_start_date = datetime.utcnow()
            data_owner_phase.started_by = current_user.user_id
            if request_body.planned_start_date:
                data_owner_phase.planned_start_date = request_body.planned_start_date
            if request_body.planned_end_date:
                data_owner_phase.planned_end_date = request_body.planned_end_date
            
            # Create DataOwnerAssignment records for non-primary key scoped attributes
            await create_data_owner_assignments(db, cycle_id, report_id, current_user.user_id)
            
            await db.commit()
            
            # Create audit log
            await create_audit_log(
                db, cycle_id, report_id, "start_data_owner_phase", "WorkflowPhase", data_owner_phase.phase_id,
        ...
        # Create data owner phase workflow
        ...
        # Create audit log
        await create_audit_log(
            db, cycle_id, report_id, "start_data_owner_phase", "WorkflowPhase", new_phase.phase_id,
            current_user.user_id, notes=request_body.notes, request=request
        )
        ...
        logger.error(f"Error starting data owner phase: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to start data owner identification phase"
        )

async def _get_data_owner_phase_status_internal(
    cycle_id: int,
    report_id: int,
    db: AsyncSession
) -> DataProviderPhaseStatus:
    """Internal helper function to get data owner phase status without permission checks"""
    try:
        logger.info(f"Starting status check for cycle_id={cycle_id}, report_id={report_id}")
        
        # Get attributes that need data owner assignment (only scoped non-primary key attributes)
        try:
        ...
        # Handle case where no attributes require data owner assignment yet
        if total_attributes == 0:
            try:
                logger.info("No attributes found for data owner phase, checking phase status...")
                # Get phase status
        ...
        try:
            logger.info("Querying data owner assignments...")
            ...
            logger.info(f"Found {len(data_owner_assignments)} data owner assignments")
        except Exception as e:
            logger.error(f"Error querying data owner assignments: {str(e)}")
            raise

        try:
            logger.info("Calculating metrics...")
            # Calculate metrics - only non-primary key attributes need data owner assignment
            ...
            # Count attributes that have data owners assigned
            ...
            # Count assignments that exist but don't have data owners yet
            ...
            logger.info(f"Metrics: Total={total_attributes}, Non-PK={total_non_pk_attributes}, LOB={attributes_with_lob_assignments}, DP={attributes_with_data_owners}, Pending={pending_cdo_assignments}, Overdue={overdue_count}")
        except Exception as e:
        ...
        logger.error(f"Error getting data owner phase status: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get data owner phase status"
        )

@router.get("/{cycle_id}/reports/{report_id}/status", response_model=DataProviderPhaseStatus)
@require_permission("data_owner", "read")
async def get_data_owner_phase_status(
    cycle_id: int,
    report_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: Any = Depends(get_current_user)
):
    """Get data owner identification phase status"""
    return await _get_data_owner_phase_status_internal(cycle_id, report_id, db)
...
async def notify_cdos_for_assignments(db: AsyncSession, cycle_id: int, report_id: int, notified_by: int):
    """Send notifications to CDOs for data owner assignments"""
    try:
...
async def get_historical_assignments(
    ...
    """Get historical data owner assignment suggestions"""
    try:
...
async def get_cdo_assignments(
    ...
    """Get data owner assignments made by the current CDO"""
    try:
        ...
        assignments = await db.execute(
            ...
            .where(and_(
                DataOwnerAssignment.cycle_id == cycle_id,
                DataOwnerAssignment.report_id == report_id,
                DataOwnerAssignment.assigned_by == current_user.user_id
            ))
...
async def get_all_cdo_assignments(
    ...
    try:
        ...
        assignments = await db.execute(
            ...
            .where(DataOwnerAssignment.assigned_by == current_user.user_id)
            .order_by(DataOwnerAssignment.assigned_at.desc())
        )
        assignments = assignments.all()

        assignment_list = []
        for dp_assignment, attribute, data_owner, lob, cycle, report in assignments:
            assignment_list.append({
                "assignment_id": dp_assignment.assignment_id,
...
async def submit_cdo_assignments(
    ...
    """CDO submits data owner assignments"""
    start_time = time.time()
    ...
    try:
        ...
        for assignment_req in request_body.assignments:
            ...
            attribute = await db.execute(
                ...
                .where(and_(
                    ...
                    ReportAttribute.is_primary_key == False  # Only non-primary key attributes need data owners
                ))
            )
            attribute = attribute.scalar_one_or_none()
            ...
            # Verify data owner belongs to same LOB
            ...
            if not data_owner:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Data provider {assignment_req.data_owner_id} not found in your LOB"
                )
            ...
            existing_assignment = await db.execute(
                ...
                .where(and_(
                    DataOwnerAssignment.cycle_id == cycle_id,
                    DataOwnerAssignment.report_id == report_id,
                    DataOwnerAssignment.attribute_id == assignment_req.attribute_id
                ))
            )
            ...
            if existing_assignment:
                ...
                existing_assignment.status = 'Completed'  # CDO task is complete once data owner is assigned
                ...
                new_assignment = DataOwnerAssignment(
                    ...
                    assigned_by=current_user.user_id,
                    assigned_at=current_time,
                    status='Completed'  # CDO task is complete once data owner is assigned
                )
        ...
        # Create audit log
        await create_audit_log(
            db, cycle_id, report_id, "submit_cdo_assignments", "DataOwnerAssignment", None,
            current_user.user_id,
...
async def get_assignment_matrix(
    ...
    """Get complete assignment matrix for data owner phase"""
    try:
        ...
        # Get attributes for data owner phase: 
        ...
        for attribute in attributes:
            ...
            sample_lobs = await db.execute(
                ...
                .where(and_(
                    ...
                    SampleRecord.sample_data.has_key(attribute.attribute_name)  # Sample contains this attribute
                ))
            )
            ...
            # Primary key attributes don't need data owner assignment but should show LOBs
            if attribute.is_primary_key:
                assignment_status = AttributeAssignmentStatus(
                    ...
                    assignment_notes="Primary key attribute - no data owner needed",
                    is_overdue=False,
                    sla_deadline=None,
            ...
            # For scoped non-primary key attributes, get data owner assignment
            ...
            if data_owner_assignment:
                ...
                if dp_assignment.data_owner_id:
                    # If a data owner is assigned, the CDO's task is complete
                    assignment_status_val = AssignmentStatus.COMPLETED
                else:
                    # If no data owner assigned yet, it's still assigned to CDO
        ...
        all_lobs = await db.execute(
            ...
            .where(Role.role_name == 'Data Executive')
        )
        all_lobs = all_lobs.scalars().all()
        
        # Get available data owners for CDO assignment interface
        # Need to join with roles table to find Data Owner role users
        data_owners = await db.execute(
            ...
            .join(Role, UserRole.role_id == Role.role_id)
            .where(and_(
                Role.role_name == 'Data Owner',
                User.is_active == True,
                User.lob_id == current_user.lob_id  # Only show data owners from CDO's LOB
            ))
        )
        data_owners = data_owners.all()
        
        # Format data owners for response
        data_owner_list = []
...
async def get_sla_violations(
    ...
    try:
        violations = await db.execute(
            select(DataOwnerSLAViolation, ReportAttribute, User, LOB)
            .join(ReportAttribute, DataOwnerSLAViolation.attribute_id == ReportAttribute.attribute_id)
            .join(User, DataOwnerSLAViolation.cdo_id == User.user_id)
            .join(LOB, User.lob_id == LOB.lob_id)
            .where(and_(
                DataOwnerSLAViolation.cycle_id == cycle_id,
                DataOwnerSLAViolation.report_id == report_id,
            ...
            .order_by(DataOwnerSLAViolation.hours_overdue.desc())
        )
...
async def send_escalation_email(
    ...
    try:
        ...
        violations = await db.execute(
            select(DataOwnerSLAViolation, ReportAttribute, User)
            .join(ReportAttribute, DataOwnerSLAViolation.attribute_id == ReportAttribute.attribute_id)
            .join(User, DataOwnerSLAViolation.cdo_id == User.user_id)
            .where(DataOwnerSLAViolation.violation_id.in_(request_body.violation_ids))
        )
        violations = violations.all()
        ...
        escalation_log = DataOwnerEscalationLog(
            cycle_id=cycle_id,
            report_id=report_id,
        ...
        # Create audit log
        await create_audit_log(
            db, cycle_id, report_id, "send_escalation_email", "DataOwnerEscalationLog", escalation_log.email_id,
            current_user.user_id,
...
async def complete_data_owner_phase(
    cycle_id: int,
    report_id: int,
    ...
    try:
        ...
        status_response = await _get_data_owner_phase_status_internal(cycle_id, report_id, db)
        ...
        if not phase:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Data owner phase not found"
            )

        # Update using workflow orchestrator for proper state management
        from app.services.workflow_orchestrator import WorkflowOrchestrator
        orchestrator = WorkflowOrchestrator(db)
        await orchestrator.update_phase_state(
            cycle_id=cycle_id,
            report_id=report_id,
            phase_name='Data Provider ID',
            new_state='Complete',
            notes=request_body.completion_notes,
        ...
        # Create audit log
        await create_audit_log(
            db, cycle_id, report_id, "complete_data_owner_phase", "WorkflowPhase", phase.phase_id,
        ...
        logger.error(f"Error completing data owner phase: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to complete data owner phase"
        )

@router.get("/{cycle_id}/reports/{report_id}/audit-log", response_model=DataProviderAssignmentAuditLog)
@require_permission("data_owner", "read")
async def get_data_owner_audit_log(
    cycle_id: int,
    report_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: Any = Depends(get_current_user)
):
    """Get data owner phase audit log"""
    try:
        audit_entries = await db.execute(
            select(DataOwnerPhaseAuditLog, User)
            .join(User, DataOwnerPhaseAuditLog.performed_by == User.user_id)
            .where(and_(
                DataOwnerPhaseAuditLog.cycle_id == cycle_id,
                DataOwnerPhaseAuditLog.report_id == report_id
            ))
            .order_by(DataOwnerPhaseAuditLog.performed_at.desc())
        )
...
async def create_data_owner_assignments(
    db: AsyncSession,
    cycle_id: int,
    report_id: int,
    created_by: int
    ...
    try:
        ...
        logger.info(f"Creating DataOwnerAssignment records for {len(scoped_attributes)} attributes")
        
        # Get approved sample records to determine LOB assignments
        ...
        # Create assignment records for each attribute and each LOB
        for attribute in scoped_attributes:
            for lob_name in all_lobs_in_samples:
                if lob_name in all_lobs:
                    ...
                    if cdo:
                        # Check if assignment already exists
                        existing_assignment = await db.execute(
                            select(DataOwnerAssignment).where(and_(
                                DataOwnerAssignment.cycle_id == cycle_id,
                                DataOwnerAssignment.report_id == report_id,
                                DataOwnerAssignment.attribute_id == attribute.attribute_id,
                                DataOwnerAssignment.lob_id == lob.lob_id
                            ))
                        )
                        existing_assignment = existing_assignment.scalar_one_or_none()
                        ...
                        if not existing_assignment:
                            ...
                            db.add(assignment)
                            logger.info(f"Created assignment for attribute {attribute.attribute_name} (ID: {attribute.attribute_id}), LOB {lob_name} (ID: {lob.lob_id}), CDO {cdo.first_name} {cdo.last_name} (ID: {cdo.user_id})")
                        else:
                            logger.info(f"Assignment already exists for attribute {attribute.attribute_name} (ID: {attribute.attribute_id}), LOB {lob_name}")
                    else:
                        logger.warning(f"No CDO found for LOB {lob_name} (ID: {lob.lob_id})")
                else:
                    logger.warning(f"LOB '{lob_name}' not found in database")
        
        logger.info("DataOwnerAssignment records created successfully")
        
    except Exception as e:
        logger.error(f"Error creating DataOwnerAssignment records: {str(e)}")
        raise
