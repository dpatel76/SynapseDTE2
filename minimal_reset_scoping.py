#!/usr/bin/env python3
"""
Minimal scoping reset script for testing - just resets version and attributes
"""
...
async def minimal_reset_scoping(cycle_id: int, report_id: int):
    """Minimal reset - just version and attributes"""
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            print(f"\n⚠️  Note: You may need to manually reset the Universal Assignment if needed.")
...
async def main():
    if len(sys.argv) != 3:
        print("Usage: python minimal_reset_scoping.py <cycle_id> <report_id>")
        print("Example: python minimal_reset_scoping.py 55 156")
        sys.exit(1)
    ...
    await minimal_reset_scoping(cycle_id, report_id)
