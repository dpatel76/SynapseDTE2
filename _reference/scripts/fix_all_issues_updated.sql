-- Fix All Issues Script (Updated with correct table names)
...
WHERE code = 'upload_data_files' AND phase_name = 'Data Profiling';

-- Update existing activity states that might be stuck
UPDATE workflow_activities
SET code = 'generate_profiling_rules'
WHERE code = 'generate_profile';
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_name = 'cycle_report_planning_pde_classifications' 
        AND column_name = 'information_security_classification'
    ) THEN
        ALTER TABLE cycle_report_planning_pde_classifications 
...
ON cycle_report_planning_pde_classifications(pde_mapping_id);

CREATE INDEX IF NOT EXISTS idx_pde_classifications_info_sec_class 
ON cycle_report_planning_pde_classifications(information_security_classification);

CREATE INDEX IF NOT EXISTS idx_pde_mappings_cycle_report 
ON cycle_report_planning_pde_mappings(cycle_id, report_id);
...
CREATE TABLE IF NOT EXISTS pde_classification_audit (
    ...
    classification_id INTEGER NOT NULL REFERENCES cycle_report_planning_pde_classifications(id),
...
DROP TRIGGER IF EXISTS trg_auto_skip_upload_files ON workflow_activities;
...
-- 8. Remove Report Owner Approval for PDE Mappings
-- ================================================

-- Remove any approval requirements from PDE mappings
-- This is handled in the application logic, but we can ensure the table doesn't enforce it

-- Drop any approval-related constraints if they exist
ALTER TABLE cycle_report_planning_pde_mappings 
DROP CONSTRAINT IF EXISTS pde_mappings_require_approval;

-- Update any existing mappings to not require approval
UPDATE cycle_report_planning_pde_mappings
SET mapping_confirmed_by_user = true
WHERE mapping_confirmed_by_user = false;
...
BEGIN
    ...
    RAISE NOTICE '8. PDE mappings no longer require report owner approval';
    RAISE NOTICE '9. Role permissions updated';
END $$;