-- Create enum types for review status and action types if they don't exist
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'reviewactiontype') THEN
        CREATE TYPE reviewactiontype AS ENUM ('submit_for_review', 'approve', 'reject', 'request_revision', 'revise', 'auto_approve');
    END IF;
END$$;

-- Create PDE Mapping Reviews table
CREATE TABLE IF NOT EXISTS pde_mapping_reviews (
    id SERIAL PRIMARY KEY,
    pde_mapping_id INTEGER NOT NULL REFERENCES cycle_report_pde_mappings(id),
    
    -- Review status
    review_status reviewstatus DEFAULT 'pending' NOT NULL,
...
CREATE TABLE IF NOT EXISTS pde_mapping_review_history (
    id SERIAL PRIMARY KEY,
    pde_mapping_id INTEGER NOT NULL REFERENCES cycle_report_pde_mappings(id),
...
CREATE TABLE IF NOT EXISTS pde_mapping_approval_rules (
    id SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_pde_mapping_reviews_mapping_id ON pde_mapping_reviews(pde_mapping_id);
CREATE INDEX IF NOT EXISTS idx_pde_mapping_reviews_status ON pde_mapping_reviews(review_status) WHERE is_latest = TRUE;
CREATE INDEX IF NOT EXISTS idx_pde_mapping_reviews_submitted_by ON pde_mapping_reviews(submitted_by_id);
CREATE INDEX IF NOT EXISTS idx_pde_mapping_reviews_reviewed_by ON pde_mapping_reviews(reviewed_by_id);

CREATE INDEX IF NOT EXISTS idx_pde_mapping_review_history_mapping_id ON pde_mapping_review_history(pde_mapping_id);
CREATE INDEX IF NOT EXISTS idx_pde_mapping_review_history_review_id ON pde_mapping_review_history(review_id);
CREATE INDEX IF NOT EXISTS idx_pde_mapping_review_history_action_by ON pde_mapping_review_history(action_by_id);

CREATE INDEX IF NOT EXISTS idx_pde_mapping_approval_rules_cycle_report ON pde_mapping_approval_rules(cycle_id, report_id) WHERE is_active = TRUE;
...
ADD COLUMN IF NOT EXISTS latest_review_status reviewstatus,
...
DROP TRIGGER IF EXISTS update_pde_mapping_review_status_trigger ON pde_mapping_reviews;
